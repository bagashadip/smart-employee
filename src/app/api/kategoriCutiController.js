...
module.exports = {
  ...
  list: async (req, res) => {
    const mKategoriCuti = await KategoriCuti.findAll({
      attributes: ["id_kategoricuti", "kode_kategoricuti", "keterangan_kategoricuti"],
    });
  ...
  create: async (req, res) => {
    ...
    res.json({ 
      status: true,
      statusCode: 200, 
      message: "Kategori Cuti " + kategoriCuti.kode_kategoricuti + " berhasil ditambah."
    });
  },
  // Update
  update: async (req, res) => {
    ...
    res.json({ 
      status: true ,
      statusCode: 200,
      message: "Kategori Cuti " + req.query.kode_kategoricuti + " berhasil diubah."
    });
  },
  // Delete
  ...
  validate: (type) => {
    ...
    const ruleKeteranganKategoriCuti = body("keterangan_kategoricuti").trim().notEmpty();

    switch (type) {
      case "create":
        {
            return [
                ruleCreateKodeKategoriCuti,
                ruleKeteranganKategoriCuti,
            ];
        }
        break;
      case "update":
        {
            return [
              ruleKodeKategoriCuti,
              ruleKeteranganKategoriCuti.optional()
            ];
        }
        break;
      case "get":
